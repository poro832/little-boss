...
각 문제를 **"왜 이 방식을 택했는가 → 결과"** 중심으로 정리했습니다.

### 1. 대용량 업로드 실패 → presigned 직접 업로드
- **문제** — 파일이 API Gateway를 통과하며 **약 10MB** 페이로드 한도에 막혀 업로드 실패. 코드가 아닌 인프라 제약.
- **해결** — 브라우저가 백엔드에서 발급받은 **presigned URL로 S3에 직접 PUT**(API Gateway 우회). 발급 실패 시 고아 레코드가 남지 않도록 'URL 발급 → 레코드 저장' 순서를 조정하고, 업로드 실패 시 정리 로직까지 설계.
- **결과** — 업로드 한도 **10MB → 50MB(5배)**, API Gateway·Lambda 페이로드 부하 제거.

### 2. 완료 알림 메일: SES 차단 → SNS 전환
- **문제** — 분석 완료를 가입 이메일로 알리려 했으나 **SES가 IAM 권한 경계로 차단**됨.
- **해결** — **SNS 토픽 + `user_id` FilterPolicy** 구독으로 전환. 가입 시 구독하고, 완료 시 해당 유저에게만 Publish.
- **결과** — 추가 권한 없이 **개인화된 완료 알림**(가입 이메일로 본인에게만) 구현.

### 3. 기능 확장 시 인프라 변경 최소화
- **문제** — '문서 완료 처리'에 새 API 라우트를 추가하면 API Gateway 메서드·CORS까지 함께 수정해야 함.
- **해결** — 기존 `PATCH /checklist/{doc_id}` 라우트를 `set_completed` 분기로 **재사용**.
- **결과** — **API Gateway·CORS 변경 0**으로 완료/되돌리기 기능 추가.
